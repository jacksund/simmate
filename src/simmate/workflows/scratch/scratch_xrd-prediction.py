...
XRDCalc = XRDCalculator(wavelength='CuKa', 
                        symprec=0, 
                        debye_waller_factors=None)

xrd_data = XRDCalc.get_pattern(struct, 
                               scaled=True, 
                               two_theta_range=None) # default two_theta_range = (0,90) # set to None for all
...
from pymatgen.analysis.diffraction.xrd import WAVELENGTHS # this is just a dictionary of values!
...
def XRDsimulatespectra(xrd_data=None, logsize=1, rad_source="CuKa", peak_profile="Gaussian", K=0.9):

    x_peak = xrd_data.x
    y_peak = xrd_data.y
    grain_size = 10 ** logsize # size of the grains in nm --> so 0.1nm grain size has a logsize=-1
    ...
    for xp, yp in zip(x_peak, y_peak):
        alpha = grain_to_hwhm(tau = grain_size, 
...
all_size_data = [XRDsimulatespectra(xrd_data=xrd_data,
                                    logsize=size,
                                    rad_source="CuKa",
                                    peak_profile="Voigt",
                                    K=0.94)
...
series3 = go.Scatter(name='0.005nm',
                     x=df['0.005x'],
                     y=df['0.005y']+noise1,
                     mode='lines', # lines, markers, text, (or a combo like lines+marker)
                     marker=dict(size=12,
...
series4 = go.Scatter(name='0.01nm',
                     x=df['0.01x'],
                     y=df['0.01y']+noise2,
                     mode='lines', # lines, markers, text, (or a combo like lines+marker)
                     marker=dict(size=12,
...
series5 = go.Scatter(name='0.02nm',
                     x=df['0.02x'],
                     y=df['0.02y']+noise3,
                     mode='lines', # lines, markers, text, (or a combo like lines+marker)
                     marker=dict(size=12,
...
layout = go.Layout(width=500 * 10/9, # golden = 1.6180339887498948482
                   height=500,
                   plot_bgcolor='white',
                   paper_bgcolor='white',
                   xaxis=dict(title_text='Raman shift (cm<sup>-2</sup>)',
                              range=[15,75],
                              showgrid=False,
                              zeroline=False,
                              ticks='outside',
                              ...
                              #dtick=250,
                              tick0=1000,),
                   yaxis=dict(title_text='Intensity (a.u.)',
                              #range=[0,75],
                              showgrid=False,
                              zeroline=False,
...
fig = go.Figure(data=[series3, series4, series5, series6, series7, series8],
                layout=layout)
