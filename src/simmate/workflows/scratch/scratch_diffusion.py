...
# Prefect Server
    # Postgres meta database
    # Workflow scheduler
    # Workflow monitoring
    # Prefect Agents
    # Workflow execution
        # Dask Cluster
            # small calculations (non-vasp)
        # FireWorks Cluster
            # large calculations (vasp)
            
# Django Server
    # Postgres result database
        # Django ORM
        # Graphene (GraphQL)
        # Django REST framework (REST API)
    # Website interface (dashboards)
        # Django Templates/Views/URLs/etc.
        # Bootstrap
...
STRUCTURE TABLE
ID, Structure, Source, Source-ID, Date_added, [relations]--> Pathways, StructureCalcs
**useful extras: nsites, e_hull
**breakdown Structure to Sites and Lattice? I can do this later on when DB size is a problem too


PATHWAY TABLE
ion, index, isite, esite, image, distance, [relations]--> PathwayCalcs

Calcs can just be columns for simplicity too! Check if this makes the db larger though.

-------------

# OPTION 1 -- we constantly loop to check number of running workflows and submit new one if needed
# This requires a separate process to be constantly running. Or I can have this as a workflow that runs every few seconds.
...
#!!! THIS STEP IS WHAT WORRIES ME -- what if multiple flows grab this at the same time? How do we ensure this works?
Set that column to 'in progress' so that it's not grabbed again. This will be overwritten by the workflow itself!

Submit the workflow via Prefect
    #!!! doublecheck that there isn't an identical run...?
    connect to DB (Look into Caching this!)
    pull input data from DB
    run calc
    upload result to DB
...
# Option 2 -- We start a set number of workflows. We have the end of one workflow trigger the start of a new one.
# This doesn't require any separate running process but instead uses on the Prefect Agents/Executors

Submit the workflow via Prefect
    connect to DB (Look into Caching this!)
    priority ranking to pull out next structure (see above)
    pull input data from DB
    run calc
    upload result to DB
    launch this workflow again

OR --

We have two workflows (a kickoff one and the main one) that go back and forth
