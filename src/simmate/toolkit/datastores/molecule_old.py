...
class MoleculeStoreOLD(Datastore):
    """
    ...
    def add_dataframe(
        ...
        if "smiles" not in df.columns:
            raise Exception(
                "dataframe must have a `smiles` column to be added to MoleculeStoreOLD"
            )
        ...
        for metadata_column in cls.metadata_columns:
            if metadata_column not in df.columns:
                raise Exception(
                    ...
                    f"to MoleculeStoreOLD. Full list of metadata columns: {cls.metadata_columns}"
                )
