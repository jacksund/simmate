...
class BaderELFAnalysis(S3Task):

    command = "bader CHGCAR_empty -ref ELFCAR_empty -b weight > bader.out"
    """
    ...
    This command is modified to use the ELFCAR as the reference for determining
    zero-flux surfaces when partitioning the CHGCAR. We also use structures that
    may contain "empty atoms" in them -- this is to help with partitioning of
    electrides where electron density is not associated with any atomic orbital.
    """
