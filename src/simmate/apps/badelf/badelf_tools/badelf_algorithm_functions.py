...
# def get_grid_axes(grid, padding: int = 0):
#     """
#     Gets the the possible indices for each dimension of a padded grid.
#     e.g. if the original charge density grid is 20x20x20, this function will
#     return three arrays with integers from 0 to 21.
#     """
#     a = np.linspace(0, grid.shape[0] + (padding - 1) * 2 + 1, grid.shape[0] + padding * 2)
#     b = np.linspace(0, grid.shape[1] + (padding - 1) * 2 + 1, grid.shape[1] + padding * 2)
#     c = np.linspace(0, grid.shape[2] + (padding - 1) * 2 + 1, grid.shape[2] + padding * 2)
#     return a, b, c
...
# def get_partitioning_line(site_voxel_coord, neigh_voxel_coord, grid, method = "linear"):
#     """
...
# def get_position_from_min(elf_min_frac, site_voxel_coord, neigh_voxel_coord):
#     """
...
#     difference = [b - a for a, b in zip(site_voxel_coord, neigh_voxel_coord)]
...
#     min_pos = [(a + b) for a, b in zip(min_pos, site_voxel_coord)]
#     return np.array(min_pos)


# def get_unit_vector(site_voxel_coord, neigh_voxel_coord, lattice):
#     """
...
#     real_site_voxel_coord = get_real_from_vox(site_voxel_coord, lattice)
#     real_neigh_voxel_coord = get_real_from_vox(neigh_voxel_coord, lattice)
...
#     # the plane.
#     normal_vector = [(b - a) for a, b in zip(real_site_voxel_coord, real_neigh_voxel_coord)]
...
# def get_radius(point, site_voxel_coord, lattice):
#     """
#     Gets the distance from an atom to the minimum point along partitioning line
#     between it and another atom.
#     """
#     real_site_voxel_coord = get_real_from_vox(site_voxel_coord, lattice)
#     radius = sum([(b - a) ** 2 for a, b in zip(real_site_voxel_coord, point)]) ** (0.5)
#     return radius
...
#         site_voxel_coord = get_voxel_from_index(site_index, lattice)
...
#             neigh_voxel_coord = get_voxel_from_neigh_CrystalNN(neigh, lattice)
#             values = get_partitioning_line(site_voxel_coord, neigh_voxel_coord, grid)[1]
...
#             if check_bond_for_covalency(values) is True:
...
#     site_index, neigh, lattice: dict, site_voxel_coord: dict, grid, rough_partitioning=False
# ):
#     """
#     Function for getting the line, plane, and other information between a site
#     and neighbor.
#     """
#     neigh_voxel_coord = get_voxel_from_neigh(neigh, lattice)
...
#         site_voxel_coord, neigh_voxel_coord, grid
#     )
...
#     elf_min_vox = get_position_from_min(elf_min_frac, site_voxel_coord, neigh_voxel_coord)
...
#     plane_vector = get_unit_vector(site_voxel_coord, neigh_voxel_coord, lattice)

#     # it is also helpful to know the distance of the minimum from the site
#     distance = get_radius(plane_point, site_voxel_coord, lattice)
#     return [
#         site_index,
#         site_voxel_coord,
#         neigh,
...
#         site_voxel_coord = row[1]["site_voxel_coord"]
#         neigh_voxel_coord = row[1]["neigh_voxel_coord"]
...
#         elf_min_vox = get_position_from_min(elf_min_frac_new, site_voxel_coord, neigh_voxel_coord)
...
#                     site_index, neigh, lattice, site_voxel_coord, grid, rough_partitioning
#                 )
...
# def get_matching_site(site_voxel_coord, results, lattice, max_distance):
#     """
...
#             site_real_coord = get_real_from_vox(site_voxel_coord, lattice)
...
# def get_matching_site_near_plane(vert_coord, results, lattice):
#     # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#     # I've had a bug in the past where more than one site is found for a single
...
#         site: int = get_matching_site_near_plane(new_vert_coord, results, lattice)
#         # site returns none if no match, otherwise gives a number
#         # The site can't return as an electride site as we don't include
#         # electride sites in the partitioning results
#         if site == -1:
...
# def get_voxel_vertices(vox_coord, results, lattice, permutations):
#     """
...
# def get_voxel_plane_intersections(
#     sites: list, results, vertices_coords: dict, intersections_df
# ):
#     """
...
#     vertices_coords, vertices_sites = get_voxel_vertices(
...
#             intersections_df = get_voxel_plane_intersections(
