...
from simmate.apps.badelf.core import (
    Grid, 
    VoxelAssignmentToolkit, 
    PartitioningToolkit, 
    ElectrideFinder,
    ZeroFluxToolkit,
    )
...
class BadElfToolkit:
    ...
    def _get_voxel_assignments(self):
        ...
        # Get the objects that we'll need to assign voxels.
            ...
            voxel_charges = [float(charge_data[idx[0], idx[1], idx[2]]) for idx in voxel_coords]
            ...
            voxel_assignments = pd.DataFrame(voxel_coords, columns=["x", "y", "z"]).add(1)
            ...
            if algorithm == "badelf":
                voxel_assignments = self._get_zero_flux_electride_assignment(voxel_assignments)
            
            voxel_assignments = self._get_plane_seperated_voxels_assignment(voxel_assignments)
            ...
            for row in voxel_assignments.iterrows():
                ...
                if row[1][4] is not None:
                    for site in row[1][4].keys():
                        if site == -1:
                            vert_multi_site_same_trans.append([row[1][0], row[1][1], row[1][2]])
                            error_indices.append(row[0])
                        elif site == -2:
                            vert_multi_site.append([row[1][0], row[1][1], row[1][2]])
                            error_indices.append(row[0])
                        elif site == -3:
                            multi_site_no_plane.append([row[1][0], row[1][1], row[1][2]])
                            error_indices.append(row[0])
            ...
            voxel_assignments['site'] = np.where( #assigns values based on boolean value. returns array
                voxel_assignments.index.isin(error_indices), #Is True where df index is in indices list
                None, # The value to be assigned
                voxel_assignments['site']) # Value to assign where condition is False
            
            voxel_assignments = self._get_multi_plane_voxel_assignment(voxel_assignments)
    ...
    def _get_zero_flux_electride_assignment(
        ...
        for electride in electride_indices:
            ...
            electride_charge = Grid.from_file(directory/f"BvAt{str(electride+1).zfill(4)}.dat").total
            electride_indices = []
            ...
            for index, row in voxel_assignments.iterrows():
                charge_density = float(electride_charge[row["x"]-1, row["y"]-1, row["z"]-1])
                if charge_density != 0:
            ...
            voxel_assignments['site'] = np.where( #assigns values based on boolean value. returns array
                voxel_assignments.index.isin(electride_indices), #Is True where df index is in indices list
    ...
    def get_electride_dimensionality(
        ...
        if self.algorithm == "zero-flux":
            ...
            # and are 0 otherwise
            elf_array = np.where(np.isin(voxel_assignment_array, electride_indices), elf_grid.total, 0)
        ...
        for index, neighbors in enumerate(nearest_neighbors):
            ...
            for neighbor in neighbors:
                ...
                pos, values = partitioning_tools.get_partitioning_line(site_pos, neigh_pos)
    ...
    def write_species_file(
        ...
        if grid.diff is not None:
            diff_array = np.where(np.isin(voxel_assignment_array, indices), grid.diff, 0)
