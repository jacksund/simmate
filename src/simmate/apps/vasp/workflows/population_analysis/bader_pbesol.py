...
from simmate.apps.vasp.workflows.static_energy.matproj_pbesol import (
...
class PopulationAnalysis__VaspBader__BaderMatprojPbesol(Workflow):
    """
    Runs a static energy calculation using an extra-fine FFT grid and PBEsol
    ...
    def run_config(
        ...
        **kwargs,
        ...
        prebader_result = StaticEnergy__Vasp__PrebaderMatprojPbesol.run(
            structure=structure,
            command=command,
            source=source,
            directory=directory,
...
class StaticEnergy__Vasp__PrebaderMatprojPbesol(StaticEnergy__Vasp__MatprojPbesol):
    """
    Runs a static energy calculation with a high-density FFT grid under settings
    from the Materials Project using the PBEsol functional. Results can be used for
    Bader analysis.
    ...
    incar = StaticEnergy__Vasp__MatprojPbesol.incar.copy()
    incar.update(
        NGXF__density_a=20,
        NGYF__density_b=20,
        NGZF__density_c=20,
