...
from simmate.apps.vasp.workflows.static_energy.matproj_scan import (
...
class PopulationAnalysis__VaspBader__BaderMatprojScan(Workflow):
    """
    ...
    def run_config(
        ...
        **kwargs,
        ...
        prebader_result = StaticEnergy__Vasp__PrebaderMatprojScan.run(
            structure=structure,
            command=command,
            source=source,
            directory=directory,
...
class StaticEnergy__Vasp__PrebaderMatprojScan(StaticEnergy__Vasp__MatprojScan):
    """
    ...
    incar = StaticEnergy__Vasp__MatprojScan.incar.copy()
    incar.update(
        NGXF__density_a=20,
        NGYF__density_b=20,
        NGZF__density_c=20,
