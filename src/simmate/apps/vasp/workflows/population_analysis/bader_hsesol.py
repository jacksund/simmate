...
from simmate.apps.vasp.workflows.static_energy.matproj_hsesol import (
...
class PopulationAnalysis__VaspBader__BaderMatprojHsesol(Workflow):
    """
    Runs a static energy calculation using an extra-fine FFT grid and HSEsol
    ...
    def run_config(
        ...
        **kwargs,
        ...
        prebader_result = StaticEnergy__Vasp__PrebaderMatprojHsesol.run(
            structure=structure,
            command=command,
            source=source,
            directory=directory,
...
class StaticEnergy__Vasp__PrebaderMatprojHsesol(StaticEnergy__Vasp__MatprojHsesol):
    """
    Runs a static energy calculation with a high-density FFT grid under settings
    from the Materials Project using the HSEsol functional. Results can be used for
    Bader analysis.
    ...
    incar = StaticEnergy__Vasp__MatprojHsesol.incar.copy()
    incar.update(
        NGXF__density_a=20,
        NGYF__density_b=20,
        NGZF__density_c=20,
