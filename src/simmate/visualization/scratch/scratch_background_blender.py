...
def example_function(lattice, sites_to_draw, save_path):
    ...
    for site in sites_to_draw:
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=4, radius=site_size*0.75, location=site_loc) # creates and sets to active object
    ...
    #make lattice black
    mat = bpy.data.materials.new(name="Lattice") # create material # can be grabbed later with bpy.data.materials['Test-Material']
...
def main():
    ...
    parser.add_argument(
        "-l", "--lattice", dest="lattice", type=str, required=True,
        help="The lattice matrix to draw (3x3 matrix to json dump)",
    )
    
    parser.add_argument(
        "-s", "--sites", dest="sites_to_draw", type=str, required=True,
