...
class Command(BaseCommand):
    ...
    def check_initials_directories(
        ...
        if not paths["components"].exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created your first component in '{app_directory.name}'! ✨\n"
                )
            )
            is_first = True
        ...
        if not paths["templates"].exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created your first template in '{app_directory.name}'! ✨\n"
                )
            )
            is_first = True
    ...
    def create_nested_directories(
        self, paths: Dict[str, Path], nested_path: str
    ) -> None:
        """
    ...
    def create_component_and_template(
        self, paths: Dict[str, Path], nested_path: str, component_name: str
    ) -> None:
        """
        ...
        component_path = (
            paths["components"] / nested_path / f"{snake_case_component_name}.py"
        )
