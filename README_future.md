...
> :warning: Several components of our software are still at the early stages for building! This means that organization of our code changes frequently, and further, some modules may be lacking full guides and documentation. We place warnings were this is relevent.

> :warning: For Simmate's workflow module, we are currently reliant on VASP, which is an expensive DFT software that can be difficult to install for beginners. We are working to move away from propriatary softwares and toward free/open-source codes like ABINIT, Quantum Espresso, or DFTK.jl. That way, you can just install Simmate and we'll take care of the rest. This will take time though... so we recommend that fully experimental labs wait until Simmate hits this milestone. If you'd like to be notified when this occurs, send me an email (simmate-team@gmail.com).
...
# Download ALL structures and store them in your local database. This
# can take some time on your first download, but you never have to 
# call this function again! After that, data access is extremely fast.
